...
[原帖链接](https://linux.do/t/topic/1061683/1)

**作者：KTMan**  
**时间：Oct 19, 2025 8:26 am**  

解决了 2 个主要痛点。

1. gpt5 输出风格死板，很严谨死板、喜欢长篇大论，重点不突出。
2. mcp serena 每次启动都要弹出网页的问题。

## AGENTS.md
> 这个是用户级别的，也就是全局可用的。位置放：`～/.codex/` 下。  
> 看了很多分享，结合了下，找到了适合自己的，请大家根据自身需求进行调整。
...
个人系统是 Mac，这套MCP 在 WSL经过测试，仍然可用。  

纯 Windows 环境，以下配置的启动命令可能需要修改，推荐在 WSL中使用。
> ~/.codex/config.toml 中部分配置  
> **exa 有免费额度，走这申请自己的,然后替换`EXA_API_KEY`：[Exa API Dashboard](https://dashboard.exa.ai/api-keys)**
